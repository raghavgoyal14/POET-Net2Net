...
def run_po_batch_fiber(iteration, optim_id, batch_size, rs_seed, noise_std):
    ...
    niche = fiber_get_niche(iteration, optim_id)
    theta = fiber_get_theta(iteration, optim_id)
    # logger.debug(f"{theta}, {theta[int(theta[0]):]}")
    noise_inds = np.asarray([noise.sample_index(random_state, len(theta[int(theta[0]):]))
    ...
    # logger.debug(f"{theta[:int(theta[0])]}")

    # logger.debug(f"{theta[int(theta[0]):] + noise_std * noise.get(noise_inds[0], len(theta[int(theta[0]):]))}")
    # logger.debug(f"{np.concatenate((theta[:int(theta[0])], theta[int(theta[0]):] + noise_std * noise.get(noise_inds[0], len(theta[int(theta[0]):]))))}")
    ...
    returns[:, 0], lengths[:, 0] = niche.rollout_batch(
        (
            np.concatenate((theta[:int(theta[0])], theta[int(theta[0]):] + noise_std * noise.get(noise_idx, len(theta[int(theta[0]):]))))
...
class ESOptimizerNet2Net:
    def __init__(self,
        ...
        logger.debug('Optimizer {} optimizing {} parameters'.format(
            optim_id, len(theta[int(theta[0]):])))
        self.optimizer = Adam(self.theta[int(self.theta[0]):], stepsize=learning_rate)
    ...
    def compute_grads(self, step_results, theta):
        ...
        po_theta_max = np.concatenate((theta[:int(theta[0])], theta[int(theta[0]):] + noise_sign * self.noise_std * noise.get(po_noise_ind_max, len(theta[int(theta[0]):]))))
        ...
        grads, _ = batched_weighted_sum(
            ...
            (noise.get(idx, len(theta[int(theta[0]):])) for idx in noise_inds),
            batch_size=500)
    ...
    def set_theta(self, theta, reset_optimizer=True):
        ...
        if reset_optimizer:
            self.optimizer.reset(self.theta[int(self.theta[0]):])
    ...
    def get_step(self, res, propose_with_adam=True, decay_noise=True, propose_only=False):
        ...
        if not propose_only:
            update_ratio, theta[int(theta[0]):] = self.optimizer.update(
                theta[int(theta[0]):], -grads + self.l2_coeff * theta[int(theta[0]):])
            ...
            if propose_with_adam:
                if len(theta) == len(self.theta):
                    update_ratio, theta[int(theta[0]):] = self.optimizer.propose(
                        theta[int(theta[0]):], -grads + self.l2_coeff * theta[int(theta[0]):])
                else:
                    update_ratio = 0.0
            else:
                update_ratio, theta[int(theta[0]):] = self.sgd_optimizer.compute(
                    theta[int(theta[0]):], -grads + self.l2_coeff * theta[int(theta[0]):])  # keeps no state
        logger.debug(
        ...
        return theta, StepStats(
            ...
            theta_norm=np.square(theta[int(theta[0]):]).sum(),
