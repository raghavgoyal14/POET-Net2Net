...
import numpy as np
from poet_distributed.es_net2net import ESOptimizerNet2Net
from poet_distributed.es_net2net import initialize_worker_fiber
from collections import OrderedDict
from poet_distributed.niches.box2d.env import Env_config
from poet_distributed.reproduce_ops import Reproducer
from poet_distributed.reproduce_ops_net2net import ReproducerNet2Net
...
def construct_niche_fns_from_env(args, env, seed):
    def niche_wrapper(configs, seed):  # force python to make a new lexical scope
        def make_niche():
            from poet_distributed.niches import Box2DNicheNet2Net
            return Box2DNicheNet2Net(env_configs=configs,
                                     seed=seed,
                                     init=args.init,
...
class MultiESOptimizerNet2Net:
    def __init__(self, args):
        ...
        self.net2net_reproducer = ReproducerNet2Net(args)
        ...
        if iteration > 0 and iteration % steps_before_adjust == 0:
            ...
            for child in child_list:
                ...
                if self.pass_mc(score_child):  # check mc
                    # mutate theta
                    if True or score_child < (self.args.mc_upper - self.args.mc_lower) / 3 + self.args.mc_lower:
                        logger.info(f"Mutating theta with score: {score_child}")
                        theta_child_mutated = self.net2net_reproducer.mutate(np.array(theta_child))
                    else:
                        theta_child_mutated = theta_child
                    # add optimizer
                    self.add_optimizer(env=new_env_config, seed=seed, created_at=iteration, model_params=np.array(theta_child_mutated))
                    admitted += 1
